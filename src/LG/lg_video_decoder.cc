...
#include "params/LGVideoDecoder.hh"
...
LGVideoDecoder::LGVideoDecoder(const LGVideoDecoderParams* params)
    : MemObject(params),
...
/*
  ...
  */
    ...
    read_dma = new LGDma(read_port, max_dma_req, inputBufferQueue, "decoder_read");
    write_dma = new LGDma(write_port, max_dma_req, outputBufferQueue, "decoder_write");
...
void
LGVideoDecoder::Initialize()
{
    schedule(tickEvent, 500000000000);
}

BaseMasterPort&
LGVideoDecoder::getMasterPort(const std::string& if_name,
        PortID idx)
{
...
LGVideoDecoder::tick()
{
    if(isBusy) { // Active state
        ...
        if(load_flag && process_flag && store_flag) {
            ...
            DPRINTF(LGVideoDecoder, "vid_dec finish the processing(%d)\n", pktPtr->frame_num);
            pktPtr = nullptr;
            isBusy = false;
        }
    }
        ...
        if (pktPtr) {
            if (!IPResponseQueue.dcBusy) {
                DPRINTF(LGVideoDecoder, "vid_dec now start(%d)\n", pktPtr->frame_num);
                isBusy = true;
            }
        }
        else {
            if(!IPRequestQueue.empty()) {
                ...
                DPRINTF(LGVideoDecoder, "vid_dec get a signal(%d)\n", pktPtr->frame_num);
            }
        }
    }
...
LGVideoDecoder::frameProcess()
{
    ...
    if ((processBufferPtr = inputBufferQueue->getUsedBuffer())) {
        ...
        process_mark_addr = process_cur_addr + buffer_size * coding_ratio;
        return false;
    }
...
LGVideoDecoder::frameStore()
{
    ...
    if (write_cur_addr - write_start_addr < FIXED_RANGE) {
    //  if (write_last_addr - write_cur_addr < FIXED_RANGE) {
        if ((write_dma->issueDmaRequest(MemCmd::WriteReq, writeBufferPtr->start_addr, buffer_size, writeBufferPtr, true)))
        {
            write_cur_addr = writeBufferPtr->start_addr + writeBufferPtr->buffer_size;
            writeBufferPtr = nullptr;
        }
...
#else
    // fixed address mapping -- stride is 6
    int buffCount = (write_cur_addr - write_start_addr) / buffer_size;
    if (buffCount % 6 == 0) {
        if ((write_dma->issueDmaRequest(MemCmd::WriteReq, write_cur_addr, buffer_size, writeBufferPtr, true)))
        {
            write_cur_addr = write_cur_addr + buffer_size;
            writeBufferPtr = nullptr;
        }
        return false;
    }
...
void
LGVideoDecoder::resetStats()
{
    MemObject::resetStats();
}

LGVideoDecoder*
LGVideoDecoderParams::create()
{
    return new LGVideoDecoder(this);
}
